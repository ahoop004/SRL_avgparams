...
class Observation:
    ...
    def get_state(self, sumo,vehicle, step, life,  destination_loc, final_destination, distcheck, distcheck_final_dest, picked_up, done):
        """
        ...
        state.append(vloc[0]* 0.0001)
        state.append(vloc[1]* 0.0001)
        state.append(destination_loc[0]* 0.0001)
        state.append(destination_loc[1]* 0.0001)
        state.append(final_destination[0]* 0.0001)
        state.append(final_destination[1]* 0.0001)
        state.append(self.manhat_dist(vloc[0], vloc[1], destination_loc[0], destination_loc[1]))  
        state.append(self.manhat_dist(vloc[0], vloc[1], final_destination[0], final_destination[1]))
